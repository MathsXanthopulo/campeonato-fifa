-- Migration 001: schema inicial do torneio FIFA 1v1
-- Rode apenas em projeto NOVO (sem tabelas public.tournaments / matches).
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'tournaments_champion_player_id_fkey'
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'tournaments_live_match_id_fkey'
  ) then
    alter table public.tournaments
...
create policy "Public can write tournaments" on public.tournaments for all using (true) with check (true);

drop policy if exists "Public can read tournament players" on public.tournament_players;
create policy "Public can read tournament players" on public.tournament_players for select using (true);

drop policy if exists "Public can write tournament players" on public.tournament_players;
create policy "Public can write tournament players" on public.tournament_players for all using (true) with check (true);
...
create policy "Public can write matches" on public.matches for all using (true) with check (true);

insert into public.tournaments (id, slug, name, status, max_players)
values ('1', 'main', 'Champions Tito', 'setup', 14)
